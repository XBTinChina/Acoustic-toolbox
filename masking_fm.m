function M = masking_fm(tag,Sigma_Dur,SampleRate,F1,F2,D1,D2,Bandwidth,NumComps,...
    ShowComps,ShowMatchFM,ShowMatchTone,ShowEndPoints,ShowFreqs,AssignFM,WavWrite)

% masking_fm(tag,Sigma_Dur,SampleRate,F1,F2,D1,D2,Bandwidth,NumComps,ShowComps,ShowMatchFM,ShowMatchTone,...
...
%
% ShowComps:	Show indiv Freq components [0=No, 1=Yes]
% ShowMatchFM:	Shows FMSeg-Chirp-FMSeg match [0=No, 1=Yes]
% ShowMatchTone:Shows ToneSeg-Chirp-ToneSeg match [0=No, 1=Yes]
% ShowEnds:	    Show endpoints [0=No, 1=Yes]
% ShowFreqs:	Show sweep freq-bands [0=No, 1=Yes]
% AssignFM:	    Assign FM waveform to workspace [0=No, 1=Yes]
% WavWrite:	    Write wav-file [0=No, 1=Yes]
%
%  e.g., masking_fm(Sigma_Dur,44100,1000,1500,200,5,100,50,0,0,0,0,0,0,1)
%
...
WavPath='G:\Stimuli\CNNB\PsychoPhys';

% -----------------------------------------------------------

NumArgs=nargin('masking_fm');
if nargin ~= NumArgs
    help masking_fm;
    return
end
...
%Sigma_Dur=D1/5; % this scales the width of the dur-distr so that it's comparable across D1
...
Tone_WAVFileName=['Tone', WAVFileName];
Noise_WAVFileName=['Noise', WAVFileName];

Short_FM_WAVFileName=['FM', num2str(D1)];
Short_Tone_WAVFileName=['Tone', num2str(D1)];
Short_Noise_WAVFileName=['Noise', num2str(D1)];

fid=fopen(['params_',num2str(tag), WAVFileName, '.txt'], 'wt');
...
for n=1:NumSweeps
    ...
    if ShowComps==1
        if n >1
            plot(FMStimSeg(1:200)/max(FMStimSeg(1:200)),'k-','LineWidth',2.0);
            pause;
        end
        clf;
    end
    ...
    for i=1:NumComps
        ...
        if ShowComps==1
            plot(X(1:200),'r-');
            title(['Sweep ', num2str(n), ' of ', num2str(NumSweeps), ',  '...
                'Component ', num2str(i), ' of ', num2str(NumComps)]);
    ...
    if n > 1
        ...
        if ShowMatchFM==1  % shows chirp and FMSeg
            tFMSES=1:length(FMStimSegEndShort(length(FMStimSegEndShort)-length(FM_InterSweepChirp):end));
            tISC=1:length(InterSweepChirp);
            plot(tFMSES,FMStimSegEndShort(length(FMStimSegEndShort)-length(FM_InterSweepChirp):end),'b-');
            hold on;
            plot(tISC+length(FMStimSegEndShort(length(FMStimSegEndShort)-length(FM_InterSweepChirp):end)),...
                InterSweepChirpPlot,'r-');
            plot(tISC+length(FM_InterSweepChirp)+length(FMStimSegEndShort(length(FMStimSegEndShort)-...
                length(FM_InterSweepChirp):end)),FMStimSegBeginShortPlot(1:length(FM_InterSweepChirp)),'b-');
            hold off;
            title(['FM Segment ', num2str(n), ' of ', num2str(NumSweeps)]);
            pause;
        end % if ShowMatch==1
    ...
    if n > 1
        ...
        if ShowMatchTone==1  % shows chirp and ToneSeg
            tToneSES=1:length(ToneStimSegEndShort(length(ToneStimSegEndShort)-length(InterSweepChirp):end));
            tISC=1:length(InterSweepChirp);
            plot(tToneSES,ToneStimSegEndShort(length(ToneStimSegEndShort)-length(InterSweepChirp):end),'b-');
            hold on;
            plot(tISC+length(ToneStimSegEndShort(length(ToneStimSegEndShort)-length(InterSweepChirp):end)),...
                InterSweepChirpPlot,'r-');
            plot(tISC+length(InterSweepChirp)+length(ToneStimSegEndShort(length(ToneStimSegEndShort)-...
                length(InterSweepChirp):end)),ToneStimSegBeginShortPlot(1:length(InterSweepChirp)),'b-');
            hold off;
            title([' Tone Segment ', num2str(n), ' of ', num2str(NumSweeps)]);
            pause;
        end % if ShowMatch==1
...
if WavWrite==1
    ...
    if ShortNames==1
        wavwrite(FM_Train,SampleRate,16,[Short_FM_WAVFileName '_' num2str(tag)]);
    else
        wavwrite(FM_Train,SampleRate,16,FM_WAVFileName);
    end
    fprintf('\nCreated:  %s\n',[Short_FM_WAVFileName '_' num2str(tag)]);
    fprintf(' in  %s\n\n',WavPath);
    
%     if ShortNames==1
%         wavwrite(Tone_Train,SampleRate,16,Short_Tone_WAVFileName);
%     else
%         wavwrite(Tone_Train,SampleRate,16,Tone_WAVFileName);
%     end
%     fprintf('Created:  %s\n',Short_Tone_WAVFileName);
%     fprintf(' in  %s\n\n',WavPath);
%     
%     if ShortNames==1
%         wavwrite(Noise_Train,SampleRate,16,Short_Noise_WAVFileName);
%     else
%         wavwrite(Noise_Train,SampleRate,16,Noise_WAVFileName);
%     end
%     fprintf('Created:  %s\n',Short_Noise_WAVFileName);
%     fprintf(' in  %s\n',WavPath);
end
