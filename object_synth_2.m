function FM_Train = object_synth(Direction,SampleRate,F1,F2,D1,D2,Bandwidth,NumComps,...
    AssignFM)
...
for n=1:NumSweeps
    ...
    if Direction(n)==0 % inverts Freq1 & Freq2 for downward-FMs
        ...
        Freq1 = (Freq1-mean(Freq1)) + F1;
        Freq2=F2+(Bandwidth/2)*(randn(1,NumComps)-0.5);
        Freq2 = (Freq2-mean(Freq2)) + F2;
        if 0==1
        ...
        Freq2 = (Freq2-mean(Freq2)) + F1;
        Freq1=F2+(Bandwidth/2)*(randn(1,NumComps)-0.5);
        Freq1 = (Freq1-mean(Freq1)) + F2;
        if 0==1
            [LogNMean,LogNVar]=lognstat(LogNSigma,LogNMu);
